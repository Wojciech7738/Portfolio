...
void CMesh::generateSkydome(int Np) {
    for (int j = -1; j < Np / 2; ++j) {
            for (int i = 0; i <= Np; ++i) {

                float t1 = 3.14 * j / Np - 3.14 / 2;
                float t2 = 3.14 * (j + 1) / Np - 3.14 / 2;
                float p = 3.14 * 2.0f * i / Np;
                ...
                add(QVector3D(x2, y2, z2), QVector3D(x2, y2, z2),
                    QVector2D(.5f + .5f * tr2 * tx2, .5f + .5f * tr2 * tz2));

                add(QVector3D(x1, y1, z1), QVector3D(x1, y1, z1),
                    QVector2D(.5f + .5f * tr1 * tx1, .5f + .5f * tr1 * tz1));
            }
        }
...
void CMesh::generateCube(GLfloat w, GLfloat h, GLfloat d)
{
    ...
    void CMesh::loadAllMeshes() {
        ...
        mesh=new CMesh;
        mesh->generateMeshFromObjFile("resources/Shotgun.obj");
        m_meshes["Shotgun"]=mesh;

        mesh = new CMesh;
        mesh->generateSkydome(36);
        m_meshes["skydome"] = mesh;

        mesh = new CMesh;
        mesh->generateMeshFromObjFile("resources/untitled.obj");
        m_meshes["pig"] = mesh;
